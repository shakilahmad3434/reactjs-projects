...
const Modal = ({ isOpen, onClose, item }) => {
  ...
  return createPortal(
    ...
    >
      <div
        ...
        role="dialog"
        aria-modal="true"
        className={`w-full max-w-4xl h-auto md:h-4/5 rounded-lg bg-white shadow-xl transition-transform duration-300 ${
      ...
      >
        <div className="p-6 flex flex-col md:flex-row justify-between items-center gap-5 h-full">
          <div className="md:w-1/2 h-full">
            <img src={item.mImage} alt={item.title} className="w-full h-full object-cover" />
          </div>

          <div className="md:w-1/2 h-full overflow-auto pl-2">
            <h2 className="text-2xl mb-3">{item.title}</h2>
            <p className="text-xl mb-5">${item.price}</p>
            <p className="text-gray-600 text-sm mb-2">Composition contains at least: outer shell 20% of recycled cotton.</p>
  ...
  <span className="relative inline-flex size-3 rounded-full bg-sky-500"></span>
            ...
            <p className="text-sm mb-1">
              <b>Color</b><span>: {item.colors[0].colorName}</span>
            </p>
            ...
            <div className="mt-4 mb-4">
              ...
              <div className="flex items-center gap-2 border border-gray-300 rounded-md py-1 px-4">
                <span className="cursor-pointer text-2xl px-2" onClick={()=> handleQuantity("prev")}>-</span>
                <span
        ref={modelQuantity}
      ...
      />
                <span className="cursor-pointer text-2xl px-2" onClick={()=> handleQuantity("next")}>+</span>
              </div>
              
              <button className="w-full px-12 py-2 bg-white border border-gray-300 text-black font-semibold rounded-md hover:bg-zinc-800 hover:text-white transition-all duration-300 ease-in-out cursor-pointer z-20">
                Add to cart
              </button>
              </div>
            </div>
